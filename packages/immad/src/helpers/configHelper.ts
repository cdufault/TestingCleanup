...
import { ConfigFileHelper, getPortalItemDataById } from '@stratcom/lib-functions';
...
export class ConfigHelper {
    ...
    static async loadAppConfig(): Promise<AppConfig> {
        if (!this.appConfig) {
            try {
                this.appConfig = await ConfigFileHelper.loadAppConfigFile('config.json');
            } catch (error) {
    ...
    static async loadAppConfigFromPortal(): Promise<PortalAppSettings> {
        ...
        if (!appConfig) {
            console.error('Error trying to loadAppConfigFromPortal in configHelper.ts. Unable to find appConfig json.');
        }
     ...
     */
    static async loadGateAppConfigFromPortal(gateAppPortalId: string): Promise<GateDynamicConfig> {
        const appConfig: AppConfig = this.getAppConfig();
        const oauthId = appConfig.oauthAppId;
        const portal = await Portal.getDefault().load();

        !gateAppPortalId && console.error('Failed to find a GATE application portal item id.');
        const jsonData = await getPortalItemDataById(gateAppPortalId, portal.url, oauthId);
