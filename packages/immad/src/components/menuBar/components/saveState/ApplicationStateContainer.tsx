...
import {
	IUserSaveState,
	ManualClassificationInfo,
	WorkSpaceItem,
} from "../../../../interfaces/UserSaveState";
 ...
 * @constructor
 */
export default function SaveStateContainer(
	props: SaveStateContainerProps,
): JSX.Element {
	...
	const classificationItems = useAppSelector(
		(state: RootState) => state.classificationSlice.classificationItems,
	);
	...
	const [lastSelectedMission, setLastSelectedMission] =
		useState<sessionMissionItem>();
	const [selectedObject] = useState(props.selectedMission);
	const [openNameDialog, setOpenNameDialog] = useState(false);
	const [finishedUpdating, setFinishedUpdating] = useState(false);
	const opsClocks = useAppSelector(
		(state) => state.saveStateSlice.existingClocksList,
	);
	...
	const viewType = useRef<"2D" | "3D">(
		selectedObject.viewType ? selectedObject.viewType : "3D",
	);
	...
	const portalClassification = useRef<PortalClassification>(
		defaultPortalClassificationItem,
	);
			...
			setMissionsInMenu().then(() => {
				setWorkSpaceViewSelectContextValues().then(() => {
					...
					if (parsedLastSelectedMission) {
						const lastMissionTimestamp =
							parsedLastSelectedMission.timeStamp as Date;
	 ...
	 */
	function LoadSessionStorageObject(
		parsedLastSelectedMission: sessionMissionItem,
	) {
		...
		if (parsedLastSelectedMission.viewType === "2D" && activeView === "SCENE") {
			...
			parsedLastSelectedMission.viewType === "3D" &&
			activeView === "MAP"
		) {
		...
		checkMissionForWorkspace();
		checkIfItemIdIsGroup(parsedLastSelectedMission.id).then(
			async (isItemGroup) => {
				if (isItemGroup) {
					updateGroupToScene(parsedLastSelectedMission.id).then(
						async (result) => {
							if (result) {
		...
		if (selectedMissionObject?.current?.value) {
			...
			if (!selectedMissionObject.current.value.workspaceId) {
				...
				setWorkSpaceViewSelectContextValues().then(async () => {
					const selectedValue = selectedMissionObject.current
						.value as WorkSpaceItem;
			...
			findPortalGroupByTitle(missionTitle).then((result) => {
				if (result.item[0]) {
					getPortalGroupUsers(result.item[0].id).then((result) => {
						const mgrName = result.admins.find(
							(managerName) => managerName === portalUser.username,
						);
	 ...
	 * @param selectedValue
	 */
	async function updateLastSelectedMission(
		selectedValue: __esri.PortalGroup | IItem | WorkSpaceItem,
	) {
	...
	function setSessionStorageItem() {
		...
		const sessionLastSelectedMission = JSON.parse(
			sessionStorage.getItem("lastSelectedMission") as string,
		);
				...
				if (processedReload?.length === 0) {
					...
					if (
						...
						sessionStorage.setItem(
							"lastSelectedMission",
							JSON.stringify(lastSelectedMission),
						);
					}
					const sessionIsNewer = ApplicationStateHelper.compareDate(
						sessionTimeStamp,
						selectedTimeStamp,
					);
					if (sessionIsNewer === 1) {
						...
						if (sessionLastSelectedMission?.isWorkspace) {
							selectedValue = selectedValue as WorkSpaceItem;
							if (
								sessionLastSelectedMission?.id !== selectedValue.portalItemId
							) {
								// last selection was workspace
								selectedMissionObject.current.value.id =
									sessionLastSelectedMission.id;
								selectedMissionObject.current.value.portalItemId =
									sessionLastSelectedMission.id;
								selectedMissionObject.current.value.title =
									sessionLastSelectedMission.title;
								selectedMissionObject.current.value.workspaceId = true;
							}
						} else if (
							sessionLastSelectedMission?.value?.id !==
							selectedValue.portalItemId
						) {
						...
						sessionStorage.setItem(
							"lastSelectedMission",
							JSON.stringify(selectedMissionObject.current),
						);
					} else {
	...
	async function setWorkspaceSelectedView() {
		...
		if (iUserSaveObjectJson.current.workspaces) {
			...
			const currentView =
				activeView === "SCENE" ? await getSceneView() : await getMapView();
			if (currentView && workspaceToLoad.length > 0) {
				...
				if (saveLoadContext.viewSelect === DEFAULT_WORKSPACE) {
					...
					if (
						workspaceToLoad[0].portalItemId !== parsedLastSelectedMission?.id
					) {
						await setClassificationRefs(workspaceToLoad[0].portalItemId);
						dispatch(setExistingClocksList(workspaceToLoad[0].opsClocks));
						await updateLastSelectedMission(workspaceToLoad[0]);
						selectedMissionObject.current.value.id =
							workspaceToLoad[0].portalItemId;
						selectedMissionObject.current.value.portalItemId =
							workspaceToLoad[0].portalItemId;
						selectedMissionObject.current.value.title =
							workspaceToLoad[0].missionValue;
						selectedMissionObject.current.value.workspaceId = true;
					}
				}
			} else {
	...
	async function loadDefaultMapForMission(missionToLoad: any): Promise<void> {
		...
		if (
			missionMetaData &&
			missionMetaData.immadVersion !== appConfig.immadVersion
		) {
	 ...
	 * @param missionSelect
	 */
	async function setDefaultMissionSceneView(
		...
		if (filteredMissions.current) {
			...
			saveLoadContext.setIsViewLoaded(false);
			isViewDoneUpdating.current = false;
			const missionsToLoad = filteredMissions.current.filter(
				(mission) => mission.title === missionSelect,
			);
			...
			const lastSelectedMission = JSON.parse(
				sessionStorage.getItem("lastSelectedMission") as string,
			);
			if (
				lastSelectedMission?.isWorkspace ||
				missionToLoad?.id !== lastSelectedMission?.id
			) {
			...
			console.error(
				"Error view is undefined can not set new mission selected.",
			);
		}
	}
	...
	async function setMissionsInMenu() {
		return getMissionsForUser(props.portalUser, appConfig.tags.mission).then(
			(results) => {
				filteredMissions.current = results.sort(
					(portalGroupA: PortalGroup, portalGroupB: PortalGroup) =>
				...
				for (let i = 0; i < filteredMissions.current.length; i++) {
					missionMenuItems.current.push(
						<MenuItem
							key={filteredMissions.current[i].id}
							value={filteredMissions.current[i].title}
						>
		...
		if (iUserSaveObjectJson.current.workspaces) {
			workspacesForMission = iUserSaveObjectJson.current.workspaces.filter(
				(workspace) =>
					...
					saveLoadContext.missionSelect + "_" + DEFAULT_WORKSPACE,
			);
		}
	...
	async function runGateWorkflow(value: number) {
		...
		findPortalItemById(missionMapId).then(async (result) => {
			if (result) {
				...
				enqueueSnackbar(
					"Unable to find Gate Map to save to. See log for more details",
					{
						variant: "warning",
					},
				);
	...
	async function handleSaveDialogClose() {
		...
		portalClassification.current.classificationDate = new Date()
			.toISOString()
			.slice(0, 10); // get format as yyyy-mm-dd
	...
	async function saveAsWorkspace() {
		if (!currentSavedState.current) {
			...
			saveLoadContext.setPortalIdToLoad(
			...
			if (userSaveExists) {
				...
				const result = await ApplicationStateHelper.createSavedUserFeature(
					iUserSaveObjectJson.current,
				);
				if (result === "true") {
	...
	async function saveToMissionDefaultScene() {
		...
		if (saveLoadContext.viewSelect === DEFAULT_WORKSPACE) {
			...
			const result = await findPortalGroupByTitle(
				saveLoadContext.missionSelect,
			);
			if (result) {
		...
		if (map?.portalItem?.type === "Web Scene") {
			...
			applicationStateHelper
				.saveWebScene(portalIdToSaveAs, sceneView)
			...
			applicationStateHelper
				.saveWebMap(portalIdToSaveAs, view as MapView)
				.then((resultValue) => {
					if (resultValue.id) {
						iUserSaveObjectJson.current.lastSavedPortalItemId = resultValue.id;
						setUserSaveExists(true);
	...
	async function updateSavedSandboxView() {
		...
		if (map?.portalItem?.type === "Web Map") {
			if (
				...
				const value = await applicationStateHelper.saveAsWebMap(
					workspaceName,
					view as MapView,
				);
				if (value) {
			...
			if (existingWorkspaceItem.length === 0) {
				...
				const value = await applicationStateHelper.saveAsWebScene(
					workspaceName,
					sceneView,
				);
				if (value) {
					...
					const message =
						"Error occurred saving as webScene see log for details";
					console.error(message);
					enqueueSnackbar(message, { variant: "error" });
				}
			} else {
				...
				// update portal item here with resultValue.id
				iUserSaveObjectJson.current.lastSavedPortalItemId = resultValue
					? resultValue.id
					: "";
				if (sceneViewLightingType === "virtual") {
					await setViewLightingToVirtual(sceneView);
	 ...
	 */
	const updateMissionAppData = async (
		webAppId: string,
		data: any,
	): Promise<boolean> => {
		try {
			const result: IUpdateItemResponse = await updatePortalWebApp(
				webAppId,
				JSON.stringify(data),
			);
		...
		enqueueSnackbar(
			"Error occurred updating Application Data. See console logs for more details.",
			{
				variant: "error",
			},
		);
	...
	const updateMissionDefaultManualClassifications = async () => {
		...
		if (missionId) {
			...
			const webApps = groupContent.filter(
			...
			if (webApps && webApps.length > 0) {
				...
				data.defaultManualClassification = manualClassifications.map(
					(item) => item,
				);
	 ...
	 */
	const updateMissionDefaultOpsClocks = async (
	...
	async function getUserSavedState() {
		await applicationStateHelper
			.getUserSavedState(props.portalUser)
			.then((returnValue) => {
				if (returnValue) {
					...
					iUserSaveObjectJson.current.defaultWebMapId =
						appConfig.defaultWebMapId;
					iUserSaveObjectJson.current.defaultWebSceneId =
						appConfig.defaultWebSceneId;
				}
			});
	}
	...
	async function updateIUserSaveObjectJson(): Promise<void> {
		...
		if (iUserSaveObjectJson.current.currentWorkspace) {
			...
			workSpaceItemID = saveLoadContext.missionSelect + "_" + DEFAULT_WORKSPACE; // for future workspace use  saveLoadContext.viewSelect;
		...
		if (iUserSaveObjectJson.current.workspaces) {
			...
			if (view) {
				...
				workSpaceItem.current.portalItemId =
					iUserSaveObjectJson.current.lastSavedPortalItemId;
				workSpaceItem.current.manualClassifications =
					getManualClassifications();
				workSpaceItem.current.opsClocks = opsClocks;
				iUserSaveObjectJson.current.lastSavedMission =
					saveLoadContext.missionSelect;
