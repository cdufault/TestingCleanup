...
const User = (props: UserProps): JSX.Element => {
	...
	const savedStateItem = useAppSelector(
		(state) => state.adminSettingsSlice.savedState,
	);
	...
	useEffect(() => {
		if (
			(!savedStateItem || savedStateItem?.itemId.trim() === "") &&
	...
	const initializeLayoutType = async (missionId: string): Promise<void> => {
		try {
			...
			if (userSaveState?.layout) {
				...
				setLayoutType(
					hasMissionLayout ? LayoutType.MISSION : LayoutType.DEFAULT,
				);
			}
		} catch (error) {
			...
			console.error(error);
			enqueueSnackbar(
				"Error initializing the layout type. Check the log for details.",
				{ variant: "error" },
			);
		}
	};
	...
	return (
		<Grid container columnSpacing={1} alignItems="center">
			<Grid item>
				<Hidden mdDown>
					<Button
						startIcon={<Avatar src={portalUser.thumbnailUrl} />}
						onClick={handleClick}
					>
						{portalUser.username}
					</Button>
				...
				>
					<StyledPopoutContent>
						...
						{(userRoles.Administrator !== false ||
							userRoles.MissionManager !== false) &&
							selectedMissionId && (
								<StyledRowWithPadding>
									<StyledColumn>
										<StyledButtonContainer>
						...
						{
							<ResetLayoutDialog
								missionTitle={selectedMissionId ?? null}
								onClose={() => setUpdated([])}
							/>
						}
					</StyledPopoutContent>
