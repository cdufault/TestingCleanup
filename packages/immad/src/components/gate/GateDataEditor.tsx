...
import {
	ApplicationItem,
	getAllGATEApps,
	queryRegionForGlobalId,
} from "./GateDataEditorHelper";
...
function GateDataEditor(): JSX.Element {
	...
	const [landingPageQueryResults, setLandingPageQueryResults] = useState<
		categoryQueryResult[]
	>([]);
	const [selectedLandingPageQueryResult, setSelectedLandingPageQueryResult] =
		useState<categoryQueryResult | undefined>();
	const [category, setCategory] = useState<string>("");
	const [regionSummaryQueryResult, setRegionSummaryQueryResult] = useState<
		textDateVal | undefined
	>();
	const [j2SummaryQueryResult, setJ2SummaryQueryResult] = useState<
		textDateVal | undefined
	>();
	...
	const [regionSummaryIsDirty, setRegionSummaryIsDirty] =
		useState<boolean>(true);
	const [j2SummaryIsDirty, setJ2SummaryIsDirty] = useState<boolean>(true);
	const [positionOnPageIsDirty, setPositionOnPageIsDirty] =
		useState<boolean>(true);
	...
	const [positionOnLandingPage, setPositionOnLandingPage] =
		useState<number>(99); //reverse sorted
	const [globalId, setGlobalId] = useState<string | undefined>("");
	const [cardIsVisibleOnLandingPage, setCardIsVisibleOnLandingPage] =
		useState<number>(-1);
	const [groupsCurrentUserIsMemberOf, setGroupsCurrentUserIsMemberOf] =
		useState<string[]>([]);
	const [groupTitleToIdMap] = useState<Map<string, string>>(
		new Map<string, string>(),
	);
	const { userRoles } = useContext(AppContext);
	const isAdminUser =
		userRoles.Administrator === true || userRoles.MissionManager === true;
	const [editActions, setEditActions] = useState<string[]>([""]);
	const [editingDisabled, setEditingDisabled] = useState<boolean>(false);
	const applicationItems = useAppSelector(
		(state) => state.gateCalendarEditorSlice.applicationItems,
	);
	const missionIsExercise = useAppSelector(
		(state) => state.gateCalendarEditorSlice.missionIsExercise,
	);
	...
	useEffect(() => {
		getAllGATEApps(
			...
			const sortedAppItems = appItems.sort(
				(appItemsA: ApplicationItem, appItemsB: ApplicationItem) =>
					appItemsA.title.localeCompare(appItemsB.title),
			);
			//filter out any duplicates by ID
			const filteredApplicationItems =
				filterOutApplicationItemDuplicates(sortedAppItems);
	...
	useEffect(() => {
		if (applicationItems && selectedMissionTitle !== "") {
			const currentlySelectedApp: any = applicationItems.find(
				(item) => item.title === selectedMissionTitle,
			);
			...
			if (!currentlySelectedApp) {
				...
				setErrorMessage(
					`Current mission was not found in the${
						gateLabel ? " " + gateLabel : ""
					} mission list.`,
				);
				return;
	...
	useEffect(() => {
		if (selectedEditAction !== "" && selectedMissionTitle !== "") {
			if (
				selectedEditAction === landingPageData ||
				selectedEditAction === analystComments
			) {
		...
		if (selectedMissionTitle !== "" && selectedAppId) {
			queryGlobalId(selectedMissionTitle).then((r) => {
				if (r) {
					...
					setEditingDisabled(true);
					setErrorMessage(
						"Editing is disabled.Request an Admin 'reset' the mission data.",
					);
					setAccessMessage(
	...
	useEffect(() => {
		if (positionOnPageIsDirty && selectedMissionTitle !== "") {
			const currentlySelectedMission: any = applicationItems.find(
				(item) => item.title === selectedMissionTitle,
			);
	 ...
	 */
	function filterOutApplicationItemDuplicates(
	...
	async function calcEditPrivileges(
		...
		const isMemberOfSelectedGroup = groupNamesUserIsMemberOf.find(
			(name) => name === missionTitle,
		);
		...
		const isAdminInSelectedMission = groupUsers.admins.find(
	...
	function updateUserMessages(isGroupMember: boolean) {
		if (!isGroupMember) {
			!isAdminUser &&
				setAccessMessage(
					"You do not appear on the member list for this group.",
				);
			//let IMMAD admins/mmgrs know why they can't edit everything
			isAdminUser &&
				setAccessMessage("Limited editing privileges for non-group members.");
			const appItem = applicationItems.find(
				(appObj: ApplicationItem) => appObj.title === selectedMissionTitle,
			);
			if (appItem && appItem.access !== "org") {
				setAccessMessage(
					"Limited editing privileges. Application item is not shared with the org.",
				);
			}
		} else {
			setAccessMessage("");
	 ...
	 */
	function handleSelectedMissionTitleChanged(
	 ...
	 * @param event change event
	 */
	function handleSelectedEditActionChanged(
		event: ChangeEvent<HTMLInputElement>,
	) {
		const value = event.target.value;
	 ...
	 * @param event change event
	 */
	async function handleSelectedCategoryChanged(
		event: ChangeEvent<HTMLInputElement>,
	) {
		const value = event.target.value;
		if (categoryIsDirty) {
	...
	async function fetchLandingPageData(
		...
		const keywords = missionIsExercise
			? appConfig.typekeywords.gateExercise
			: appConfig.typekeywords.gateMission;
		...
		if (regionSummaryIsDirty) {
			if (globalId) {
				...
				await fLayerAnalystComments.load().then(async () => {
					const results = await QueryRegionSummaryLib(
						fLayerAnalystComments,
						globalId ? globalId : "",
					);
		...
		if (j2SummaryIsDirty) {
			if (globalId) {
				...
				await j2SummaryFLayer.load().then(async () => {
					const results = await QueryJ2AssessmentLib(
						j2SummaryFLayer,
						globalId ? globalId : "",
					);
	...
	async function queryGlobalId(
		...
		const selectedMissionAppObj: any = applicationItems.find(
			(item) => item.title === selectedMissionTitle,
		);
		...
		if (applicationObjId) {
			...
			const message =
				"Failed to find a mission id for mission: " + selectedMissionTitle;
	...
	async function queryMetadata(
		missionAppId: string | undefined = undefined,
			...
			<WidgetContent>
				<FieldGroup>
					{applicationItems.length > 0 ? (
						<Typography variant="caption">Select A Mission</Typography>
					) : (
						""
					)}
					<InputField
				...
				<FieldGroup>
					{!selectedEditAction ? (
						<Typography variant="caption">Select An Item to Edit</Typography>
					) : (
						""
					)}
					<InputField
						...
						<FieldGroup>
							...
							>
								{landingPageQueryResults.map((queryResult) => (
									<MenuItem
										key={queryResult.category.trim()}
										value={queryResult.category.trim()}
									>
