...
function GateCalendarEditor(props: GateCalendarEditorProps): JSX.Element {
	...
	const [gateCalendarEvent, setGateCalendarEvent] = useState<GateCalendarEvent>(
	...
	const [numberOfOccurrencesRadio, setNumberOfOccurrencesRadio] = useState<
		boolean | undefined
	...
	const [calendarFeatureLayer, setCalendarFeatureLayer] =
	...
	const selectedEventToEdit = useAppSelector(
		(state) => state.gateCalendarEditorSlice.selectedEventToEdit,
	);
	let calendarFeatureLayerId = appConfig.gate.calendarGuid;
	const isUpdate = useAppSelector(
		(state) => state.gateCalendarEditorSlice.isUpdate,
	);
	const deleteFutureEvents = useAppSelector(
		(state) => state.gateCalendarEditorSlice.deleteFutureEvents,
	);
	const updateFutureEvents = useAppSelector(
		(state) => state.gateCalendarEditorSlice.updateFutureEvents,
	);
	...
	const currentMissionName = useAppSelector(
		(state) => state.gateCalendarEditorSlice.currentMissionName,
	);
	const [recurringCheckboxTooltip, setRecurringCheckboxTooltip] =
		useState<string>("");
	const [disableRecurringCheckbox, setDisableRecurringCheckbox] =
	...
	useEffect(() => {
		if (selectedEventToEdit.objectid !== "") {
			...
			const icodSelectedDate = selectedEventToEdit.icod
				? new Date(selectedEventToEdit.icod)
				: undefined;
			...
			if (
				selectedEventToEdit.recurrence_end_date &&
				selectedEventToEdit.recurring
			) {
				convertedRecurrenceEndDate = new Date(
					selectedEventToEdit.recurrence_end_date,
				);
			}
	...
	useEffect(() => {
		if (isUpdate && selectedEventToEdit.recurring === 0) {
			...
			setRecurringCheckboxTooltip(
				"Cannot make existing event recurring. Create a new recurring event series.",
			);
		} else {
	...
	useEffect(() => {
		if (applicationItems && currentMissionName !== "") {
			const currentlySelectedApp: any = applicationItems.find(
				(item) => item.title === currentMissionName,
			);
			const regionGuid = regionItems.find(
				(item) => item.title === currentMissionName,
			);
	...
	const handleLengthInDaysChange = (value: string) => {
		...
		if (newEndDate && newLengthInDays >= 0) {
			...
			if (!isNaN(newEndDate.getTime())) {
				...
				enqueueSnackbar("Invalid Date. Length of days is too long.", {
					variant: "error",
				});
			}
		}
	...
	const handleStartDateChange = (newStartDate: Date) => {
		...
		if (lengthInDays > 0 && endDate) {
			...
			const newLengthInDays = Math.floor(
	...
	const handleEndDateChange = (newEndDate: Date) => {
		...
		const adjustedEndDate = new Date(
			start.getTime() + newLengthInDays * (1000 * 60 * 60 * 24),
		);
	 ...
	 */
	const handleSelectedMissionTitleChanged = (
	...
	const handleParticipantsChange = (event: ChangeEvent<HTMLInputElement>) => {
		...
		const participantsArray = inputValue
			.split(",")
	...
	const handleParticipantsLoseFocus = () => {
		const participantsArray = gateCalendarEvent.participants.map(
	 ...
	 * @param event change event
	 */
	const handleNumberOfWeeksChange = (
		event: React.ChangeEvent<HTMLInputElement>,
	) => {
		setGateCalendarEvent({
			...gateCalendarEvent,
	...
	const handleUpdateButtonText = () => {
		if (
			...
			selectedEventToEdit.is_master_record === 1
		) {
			...
			<Dialog open={isDeleteDialogOpen}>
				...
				<DialogActions>
					<RightButton
						color="primary"
						variant="contained"
						onClick={handleDeleteEventCancelClick}
					>
						Cancel
					</RightButton>
					<RightButton
						color="secondary"
						variant="contained"
						onClick={handleDeleteEventOKClick}
					>
						OK
					</RightButton>
				</DialogActions>
			...
			<WidgetContainer>
				...
				<WidgetContainer>
					...
					<TextField
						...
						helperText={
							validDates
								? "Invalid Dates. Need date range 0 or greater in days."
								: ""
						}
						onChange={(e: ChangeEvent<HTMLInputElement>) =>
							handleLengthInDaysChange(e.target.value)
						}
					/>
					<TextInput
						label="Location"
					...
					{gateCalendarEvent.recurring &&
						recurrenceTypeOptions(gateCalendarEvent.recurrenceType)}
					{gateCalendarEvent.recurring && (
						<Box>
							...
							<RadioGroup
								value={
									numberOfOccurrencesRadio ? "afterNumOccurrences" : "endOn"
								}
								onChange={() =>
									setNumberOfOccurrencesRadio(!numberOfOccurrencesRadio)
								}
							>
								<DatePickerContainer>
									<FormControlLabel
					...
					<ICODWidget
						onDateChange={handleIcodDateChange}
