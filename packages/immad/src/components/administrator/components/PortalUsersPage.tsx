...
export default function PortalUsersPage(): JSX.Element {
    ...
    const adminObject = useRef<IMMADGroupType | undefined>({ groupName: '', groupId: '' });
    const missionManagerObject = useRef<IMMADGroupType | undefined>({ groupName: '', groupId: '' });
    const analystObject = useRef<IMMADGroupType | undefined>({ groupName: '', groupId: '' });
    ...
    }, [adminSettingsContext.rowsPerPage, adminSettingsContext.tableOrderBy, adminSettingsContext.tablePage]);
    ...
    async function initializeRefObjects() {
        const adminPromise = findPortalGroupByTitle(appConfig.roles.admin.tag).then((adminGroupResult) => {
            immadGroups.current.push({
                groupName: appConfig.roles.admin.tag,
                groupId: adminGroupResult.item[0].id,
            });
        });
        const missionManagerPromise = findPortalGroupByTitle(appConfig.roles.missionManager.tag).then(
            (missionManagerGroupResult) => {
                immadGroups.current.push({
        ...
        const analystPromise = findPortalGroupByTitle(appConfig.roles.analyst.tag).then((analystGroupResult) => {
            immadGroups.current.push({
    ...
    function updateTable() {
        ...
        if (currentFilter.current.id === 0) {
            if (
                ...
                adminSettingsContext.rowsPerPage > adminSettingsContext.filteredUsers.length
            ) {
    ...
    async function enrichUsersWithIMMADRole(loadedUsers: IUser[]): Promise<void> {
        if (
            ...
            const userItems: IIMMADUser[] =
                adminSettingsContext.filteredUsers.length > 0 ? adminSettingsContext.filteredUsers : [];
            if (loadedUsers) {
                // create user objects here and then add to setFilteredUsers in the context
                ...
                const sortedValues: IIMMADUser[] = userItems.sort(
                    compareValues(adminSettingsContext.tableOrderBy, adminSettingsContext.tableOrder)
                );
    ...
    async function updateUser(user: IUser): Promise<IIMMADUser> {
        ...
        if (user) {
            ...
            const isAdmin = userGroups.find((group) => group.id === adminObject.current?.groupId);
            const isMissionManger = userGroups.find((group) => group.id === missionManagerObject.current?.groupId);
            const isAnalyst = userGroups.find((group) => group.id === analystObject.current?.groupId);
            if (isAdmin) {
    ...
    function missionManagerFilter() {
        ...
        if (missionManagerObject.current) {
            missionManagerSearchValue +=
                '* group:' + missionManagerObject.current.groupId + ' -group:' + adminObject.current?.groupId;
        }
    ...
    function analystFilter() {
        ...
        const analystObject = immadGroups.current.find((element) => element.groupName === analystTag);
        // search value is combination of user name in search and the
        // group to search in Id's minus the groups with higher access
    ...
    function setFilterQuery(searchValue: string): void {
        ...
        if (adminSettingsContext.tablePage !== 0) {
            const checkValue = adminSettingsContext.tablePage * adminSettingsContext.rowsPerPage;
        ...
        searchPortalUserByPartialUserName(
            ...
            if (adminSettingsContext.tablePage * adminSettingsContext.rowsPerPage === 0) {
                replaceFilteredUsers(loadedUsers);
            } else {
    ...
    function compareValues(propertyKey: string, order: 'asc' | 'desc' = 'asc') {
        ...
        return function innerSort(a: any, b: any) {
            ...
            const varA = typeof a[propertyKey] === 'string' ? a[propertyKey].toUpperCase() : a[propertyKey];
            const varB = typeof b[propertyKey] === 'string' ? b[propertyKey].toUpperCase() : b[propertyKey];
            let comparison = 0;
            if (varA > varB) {
                comparison = 1;
            } else if (varA < varB) {
            ...
            <StyledSplitDivContainer>
                <StyledLeftSideDiv>
                    ...
                    <StyledMuiAccordion>
                        ...
                        <AccordionDetails>
                            <FilterListItems onFilterChange={handleFilterChanged} filtersList={filtersList} />
                        </AccordionDetails>
                    </StyledMuiAccordion>
                </StyledLeftSideDiv>
                <StyledRightSideDiv className='main'>
                    ...
                    />
                    <EnhancedTable handleIndividualUserRoleChanged={handleIndividualUserRoleChanged} />
                </StyledRightSideDiv>
            </StyledSplitDivContainer>
