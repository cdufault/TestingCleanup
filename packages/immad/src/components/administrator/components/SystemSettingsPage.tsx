...
type AppSettingsReducer = (
	state: PortalAppSettings,
	action: UserAction,
) => PortalAppSettings;

const reducer: AppSettingsReducer = (
	state: PortalAppSettings,
	action: UserAction,
) => {
	switch (action.type) {
...
export default function SystemSettingsPage(): JSX.Element {
	...
	const [immadState, dispatch] = useReducer(
		reducer,
		appConfigPortal ?? initialState,
	);

	const [isDisabled, setIsDisabled] = useState<boolean>(true);
	const [isPublishing, setIsPublishing] = useState<boolean>(false);

	const [dynamicLayerServiceId, setDynamicLayerServiceId] =
		useState<string>("");
	const [pollingInterval, setPollingInterval] = useState<number>(30);
	const [defaultExpirationTimeHrs, setDefaultExpirationTimeHrs] =
		useState<number>(24);

	const [showPublishConfirmation, setShowPublishConfirmation] =
		useState<boolean>(false);
	const gateAppIdFromSlice = useAppSelector(
		(state) => state.adminSettingsSlice.gateApplicationId,
	);
	 ...
	 */
	const getGateAppConfigFromPortal = async (
		gateAppId: string | undefined,
	): Promise<GateConfigData | null> => {
		try {
			if (gateAppId) {
				const config = await ConfigHelper.loadGateAppConfigFromPortal(
					gateAppId,
				);
				if (config) {
				...
				console.warn(
					"GATE app configuration was not found for id: " + gateAppId,
				);
			} else {
				console.warn(
					"No portal item id was provided to get GATE application ID.",
				);
			}
		} catch (error) {
			console.error(
				`Error getting GATE application config for id: ${gateAppId}. Message: ` +
					error.message,
				error,
			);
		}
	...
	async function loadDataFromGateAndImmadApplicationObjects() {
		try {
			...
			dispatch(
				actionCreator(
					"savedState",
					appConfigPortal.savedState?.itemId ?? initialState.savedState.itemId,
				),
			);
			setNewSelectedGateAppId(
			...
			setDefaultExpirationTimeHrs(
				gateConfig?.configurationData
					?.dynamicLayerServiceDefaultExpirationTimeHrs ??
					initialGateState.dynamicLayerServiceDefaultExpirationTimeHrs,
			);
			!gateConfig ??
				console.warn(
					"Gate app config was not found. Setting PUSH related values to defaults.",
				);
		} catch (error) {
	...
	async function writeNewValuesToGateAndImmadAppObject(
		...
		const result = await updatePortalWebApp(
			immadAppId,
			JSON.stringify(immadCurrentState),
		);

		let updatedWithoutError = true;
		if (result.success) {
			const gateAppConfigFromPortal = await getGateAppConfigFromPortal(
				gateAppId,
			);
			...
			if (gateAppConfigFromPortal) {
				if (gateAppConfigFromPortal?.configurationData) {
					gateAppConfigFromPortal.configurationData.dynamicLayerServiceId = {
						itemId: dynamicLayerId,
					};
					gateAppConfigFromPortal.configurationData.dynamicLayerServicePollIntervalMins =
						pollingIntervalMins;
				...
				const result = await updatePortalWebApp(
					gateAppId,
					JSON.stringify(gateAppConfigFromPortal),
				);
				console.debug(JSON.stringify(result, null, 2));
			} else {
				console.error(
					"Failed to find a GATE application object for id: " + gateAppId,
				);
			...
			// log error
			console.error(
				"Update item response was unsuccessful for item id: " + immadAppId,
			);
	...
	async function saveButtonClicked() {
		...
		if (updatedOK) {
			enqueueSnackbar("Updated System Settings Successfully", {
				variant: "success",
			});
		} else {
			enqueueSnackbar(
				"Error occurred updating system settings. See console logs for more details.",
				{
					variant: "error",
				},
			);
	...
	async function handlePublish() {
		enqueueSnackbar("Publishing Dynamic Layer Feature Service...", {
			variant: "info",
		});
		...
		try {
			...
			if (portal) {
				await portal.load();
				const userSession = await getPortalUserSession(
					portal.url,
					staticAppConfig.oauthAppId,
				);

				const layerDefJson = staticAppConfig.gate.dynamicLayerServiceTemplate;
				const featureDefJson =
					staticAppConfig.gate.dynamicFeatureServiceTemplate;
				...
				if (userSession && layerDefJson) {
					...
					if (portalItem) {
						...
						try {
							const result = await findPortalGroupsByTag(
								missionMgrTag,
								userSession,
							);
							...
							if (result.success && result.item.length > 0) {
								...
								console.warn(
									`Failed to find a group with the tag ${missionMgrTag}.`,
								);
							}
						} catch (error) {
						...
						try {
							...
							if (!result) {
								...
								enqueueSnackbar(
									"Successfully Published the Service. Select Save to commit changes.",
									{
										variant: "success",
									},
								);
			...
			<FieldGroup>
				...
				<FieldGroup>
					<Stack direction={"row"}>
						...
						<ConfirmationDialog
							description={`This will publish a new service for the${
								ConfigHelper.getAppConfig()?.gate?.gateLabel ?? ""
									? " " + ConfigHelper.getAppConfig().gate.gateLabel
									: ""
							} Push Dynamic Layer functionality. Do you wish to continue?`}
				...
				<StyledPortalItemSelectBox>
					<FieldGroup>
						<PortalItemSelect
							...
							portalItemID={
								newSelectedGateAppId ? newSelectedGateAppId : gateAppIdFromSlice
							}
