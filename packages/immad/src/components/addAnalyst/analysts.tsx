...
const Analysts = (props: {
    ...
    const { groupId, analystUpdateCallback, mgrsNamesInCreateMissionReducer, resetAnalysts } = props;
    ...
    const [userNamesAddedToMission, setUserNamesAddedToMission] = useState<string[]>([]); //users added to mission + group admins

    const [usersInGrid, setUsersInGrid] = useState<ImmadAnalyst[]>([]); //analysts displayed in the all analysts grid
    const [usersAddedToMissionInGrid, setUsersAddedToMissionInGrid] = useState<ImmadAnalyst[]>([]); //objs displayed in the selected users grid
    ...
    const immadAdminNames = useAppSelector((state: RootState) => state.missionCreationSlice.immadAdminUserNames);
    const usersAddedViaGrid = useAppSelector((state: RootState) => state.missionCreationSlice.analystsInTheMission);
    ...
    useEffect(() => {
        if (allAnalysts && users && users.length > 0 && immadAdminNames && immadAdminNames.length > 0) {
            updateUsers();
        }
    ...
    async function updateUsers() {
        ...
        updateUserRoleBasedOnType(allImmadUserObjects, adminsArray ? adminsArray : [], 'manager');
    ...
    useEffect(() => {
        if (filterString != '') {
            ...
            setUsersInGrid(filterGridBasedOnSearchString(filterString, allImmadAnalysts, userNamesAddedToMission));
     ...
     */
    const tempRefreshGrid: refreshCallback = (missionMembers: ImmadAnalyst[], userNamesInMission: string[]) => {
    ...
    function clearFilter() {
        ...
        refreshGridModel(allImmadAnalysts, tempRefreshGrid, analystUpdateCallback, true);
    }
    ...
    const currentPageRows = usersInGrid.slice(page * pageSize, (page + 1) * pageSize);
    ...
    const handleSelectionModelChange = (newSelection: any[]) => {
        ...
        const selectedOnPage = new Set(newSelection.filter((id) => currentPageRowIds.includes(id)));
