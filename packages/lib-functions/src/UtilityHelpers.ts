...
import { Extent, Geometry, Point } from '@arcgis/core/geometry';
...
import SceneView from '@arcgis/core/views/SceneView';
   ...
   */
  export function BuildMapExtentFromScreenPoint(
    currentView: MapView | SceneView,
    screenPoint: any,
    expandFactor: number = 2,
    pixelDistance: number = 3
): Geometry | undefined {
    let expandedExtent: Extent | undefined;
    if(currentView){
        const xmin = currentView.toMap({
            x: screenPoint.x - pixelDistance,
            y: screenPoint.y,
        });
        const xmax = currentView.toMap({
            x: screenPoint.x + pixelDistance,
            y: screenPoint.y,
        });
        const ymin = currentView.toMap({
            x: screenPoint.x,
            y: screenPoint.y + pixelDistance,
        });
        const ymax = currentView.toMap({
            x: screenPoint.x,
            y: screenPoint.y - pixelDistance,
        });

        if (xmin && xmax && ymin && ymax) {
            const extentFromPoint = new Extent({
                xmin: xmin.x,
                xmax: xmax.x,
                ymax: ymax.y,
                ymin: ymin.y,
            });
            expandedExtent = extentFromPoint.expand(expandFactor);
        }
    }
