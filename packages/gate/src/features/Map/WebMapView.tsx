...
const WebMapView = (): JSX.Element => {
	const mapViewSlicePortalItemId = useSelector(
		(state: RootState) => state.mapViewSlice.viewObjPortalItemId,
	);
	...
	const [currentWebScene, setCurrentWebScene] = useState<
		WebMap | WebScene | undefined
	>();
	...
	const activeViewType = useSelector(
		(state: RootState) => state.mapViewSlice.activeViewType,
	);
	const displayMode = useSelector(
		(state: RootState) => state.applicationSlice.regionDisplayMode,
	);
	const [portalItemId, setPortalItemId] = useState<string>();
	const lightingIsEnabled = useSelector(
		(state: RootState) =>
			state.applicationSlice.applicationConfig.lightingIsEnabled,
	);
	const [lastViewLoaded, setLastViewLoaded] = useState<
		SceneView | MapView | undefined
	>();
	const [isLoading, setIsLoading] = useState<boolean>(false);
	const countsLoaded = useSelector(
		(state: RootState) => state.mapViewSlice.countsWidgetInitialzied,
	);
	...
	const [errorMessageText, setErrorMessageText] = useState(
		"Failed to locate the mission map.",
	);

	const gateTypeKeywords = useSelector(
		(state: RootState) => state.applicationSlice.gateRegionAppTypeKeywords,
	);
	const webSceneToMissionNameMappings = useSelector(
		(state: RootState) => state.mapViewSlice.websceneMappings,
	);
	const cachedMissionName = useSelector(
		(state: RootState) => state.mapViewSlice.viewItemObjMissionName,
	);
	const [hasMapLoadErrorCondition, setHasMapLoadErrorCondition] =
		useState(false);
	const landingPageItems: ILandingPageItems = useSelector(
		(state: RootState) => state.landingPage.landingPageItems,
	);
     ...
     */
	const dynamicLayersRef = useRef<Map<string, string>>(
	...
	useEffect(() => {
		if (hasMapLoadErrorCondition) {
			console.log(
				"MISSING MAP. Will try to find an updated map for the mission",
			);
	 ...
	 */
	const checkForUpdatedPortalMapId = async (
		nameOfTheCurrentMission: string,
	) => {
		try {
			let updatedSceneId = "";
			if (nameOfTheCurrentMission) {
				...
				updatedSceneId = gateAppData.defaultViewId
					? gateAppData.defaultViewId
					: "";
				if (!updatedSceneId) {
					const message =
						"No mapping in the mapping table for map/scene with name: " +
						nameOfTheCurrentMission;
	...
	async function addPushedLayer(layerItemId: string, viewState: ViewState) {
		if (layerItemId) {
			...
			if (!dynamicLayersRef.current.get(layerItemId)) {
				...
				enqueueSnackbar("Added " + featureLayer.title + " to the map.", {
					variant: "success",
				});
			}
		}
		...
		if (mapLayerId) {
			...
			if (layerId) {
				...
				if (layer) {
					...
					enqueueSnackbar("Removed " + layer.title + " from the map.", {
						variant: "info",
					});
				}
			}
	...
	async function loadDynamicLayers(viewState: ViewState) {
		...
		const groupId = getGroupIdFromLandingPageItemsCache(
			regionId ? regionId : missionName,
		);
		if (!groupId) {
			console.error(
				`Failed to find a group id for: ${regionId ? regionId : missionName}`,
			); //use friendly name instead of GUID
			return;
		}
		...
		if (featureLayer && groupId) {
			console.debug(
				"GATE Dynamic Layers: Querying layer using Mission ID: " + groupId,
			);
			const ftrLayerMapping = await createPushActionMappings(
				groupId,
				dateField,
				layerItemId,
				featureLayer,
	...
	const runLoadDynamicLayersInterval = (viewState: ViewState) => {
		...
		if (dynamicConfig.dynamicLayerServicePollIntervalMins) {
			...
			console.info(
				"GATE Dynamic Layers: Polling interval set to " +
					pollingInterval +
					" minutes",
			);
		} else {
	...
	async function loadTheView(): Promise<ViewState | undefined> {
		...
		if (
			...
			if (
				currentWebScene &&
				currentWebScene.portalItem?.id === mapViewSlicePortalItemId
			) {
				...
				if (appConfig.defaultSymbol) {
					...
					try {
						if (activeViewType === "SCENE") {
							const webglSupport: WebGL3DSupportInfo =
								getWebGL3DSupportInfo() as WebGL3DSupportInfo;
						...
						if ((error as Error)?.name?.includes("not-authorized")) {
							enqueueSnackbar(
								`Insufficient privileges to view layers, contact${
									appConfig?.appLabel ? " " + appConfig.appLabel : ""
								} admin.`,
								{
									variant: "error",
									autoHideDuration: 12000,
