...
import { GateDynamicConfig, setGateConfigured, setGateDynamicConfig } from '../../ApplicationSlice';
import { findAPortalItemById, getPortalItemDataById, Logger, updatePortalWebApp } from '@stratcom/lib-functions';
import { OpsClockEditorWidgetLib, PortalItemSelect } from '@stratcom/react-widget-lib';
...
export default function ConfigurationForm() {
    ...
    const [formErrors, setFormErrors] = useState<GateDynamicConfig>({} as GateDynamicConfig);
    ...
    useEffect(() => {
        ...
        if (Object.keys(formErrors).length !== 0) {
            ...
            if (formData.opsClockList === undefined || formData.opsClockList.length === 0) {
                setFormData({
                    ...formData,
    ...
    useEffect(() => {
        ...
        if (appConfig) {
            getPortalItemDataById(appConfig.appPortalId, portal.url, appConfig.oauthAppId).then((itemData) => {
                if (itemData && itemData.configurationData) {
                    ...
                    setFormData((prevState) => ({ ...prevState, ...itemData.configurationData }));
    ...
    async function handleSubmit(event: any) {
        ...
        if (Object.keys(errors).length === 0) {
            if (appConfig) {
                ...
                if (!result?.success) {
                    Logger.log('Error Occurred updating GATE application.', 'ERROR', result);
                    navigate('/error', { state: 'Error Occurred updating GATE application.' });
                }
    ...
    async function validateInputs(configurationData: GateDynamicConfig) {
        ...
        if (!configurationData.regionFeatureClassId) {
            ...
            const result = await isValidFeatureServiceGuid(configurationData.regionFeatureClassId);
            if (!result) {
                errors.regionFeatureClassId = 'Invalid Value for Region FeatureClass Id';
            }
        }
        if (!configurationData.landingPageCategoriesFeatureClassId) {
            errors.landingPageCategoriesFeatureClassId = 'ID Required for Landing Page Categories FeatureClass Id';
        } else {
            const result = await isValidFeatureServiceGuid(configurationData.landingPageCategoriesFeatureClassId);
            if (!result) {
        ...
        if (!configurationData.j2SummaryFeatureClassId) {
            errors.j2SummaryFeatureClassId = 'ID Required for J2 Summary FeatureClass Id';
        } else {
            const result = await isValidFeatureServiceGuid(configurationData.j2SummaryFeatureClassId);
            if (!result) {
                errors.j2SummaryFeatureClassId = 'Invalid Value for J2 Summary FeatureClass Id';
            }
        }
        if (!configurationData.sourcesFeatureClassId) {
            errors.sourcesFeatureClassId = 'ID Required for Sources FeatureClass Id';
        } else {
            const result = await isValidFeatureServiceGuid(configurationData.sourcesFeatureClassId);
            if (!result) {
                errors.sourcesFeatureClassId = 'Invalid Value for Sources FeatureClass Id';
            }
        }
        if (!configurationData.analystCommentsFeatureClassId) {
            errors.analystCommentsFeatureClassId = 'ID Required for Analyst Comments FeatureClass Id';
        } else {
            const result = await isValidFeatureServiceGuid(configurationData.analystCommentsFeatureClassId);
            if (!result) {
                errors.analystCommentsFeatureClassId = 'Invalid Value for Analyst Comments FeatureClass Id';
            }
        }
        if (configurationData.analystCommentsAlias === '') {
            errors.analystCommentsAlias = 'A Value is required for Analyst Comments Alias';
        }
        ...
        if (configurationData.highInterestEventCardTitle === '') {
            errors.highInterestEventCardTitle = 'A title is required for High Interest Events';
        }
        if (configurationData.systemHighClassification === '') {
            errors.systemHighClassification = 'A classification is required for System High Classification';
        }
        if (configurationData.brandingTitleAlias === '') {
            errors.brandingTitleAlias = 'A Value is required for Branding Title Alias';
        }
        if (configurationData.brandingSubtitleAlias === '') {
            errors.brandingSubtitleAlias = 'A Value is required for Branding Subtitle Alias';
        }
        ...
        if (
            ...
            (configurationData.presentationModeUpdateIntervalMinutes as unknown as number) <= 0 ||
        ...
        if (
            ...
            (configurationData.carouselPagingUpdateIntervalMinutes as unknown as number) <= 0 ||
        ...
        if (
            ...
            (configurationData.landingPageUpdateIntervalInMinutes as unknown as number) <= 0 ||
        ...
        if (
            configurationData.updateFrequencyForAnalystCommentCategoryInMinutes === undefined ||
            (configurationData.updateFrequencyForAnalystCommentCategoryInMinutes as unknown as number) <= 0 ||
        ...
        if (!configurationData.lowActivitySnapshotCategoryColor) {
            errors.lowActivitySnapshotCategoryColor = 'Low Activity Catagory Color must be selected';
        }
        if (!configurationData.moderateActivitySnapshotCategoryColor) {
            errors.moderateActivitySnapshotCategoryColor = 'Moderate Activity Catagory Color must be selected';
        }
        if (!configurationData.highActivitySnapshotCategoryColor) {
            errors.highActivitySnapshotCategoryColor = 'High Activity Catagory Color must be selected';
        }
        return errors;
    }
     ...
     */
    function handlePortalItemChange(portalItem: PortalItem | null, parameterName: string) {
        setFormData({ ...formData, [parameterName]: portalItem ? portalItem.id : '' });
    }
    ...
    async function isValidFeatureServiceGuid(guidValue: string) {
        let isValid = false;
        if (guidValue) {
            const result = await findAPortalItemById(guidValue, portal.url, appConfig.oauthAppId);
            if (result) {
                isValid = true;
            }
        }
                ...
                <div className='gate-input-group'>
                    ...
                    <Box className={'gate-input-field'}>
                        <PortalItemSelect
                            ...
                            onItemChange={(item: PortalItem | null) => {
                                handlePortalItemChange(item, 'landingPageCategoriesFeatureClassId');
                            }}
                        />
                    </Box>
                    <Box className={'gate-input-field'}>
                    ...
                    <Box className={'gate-input-field'}>
                        <TextField
                            ...
                            variant='outlined'
                            error={Boolean(formErrors.updateFrequencyForAnalystCommentCategoryInMinutes)}
                            helperText={formErrors.updateFrequencyForAnalystCommentCategoryInMinutes}
                    ...
                    <Box className='gate-input-field category-colors-container'>
                        ...
                        >
                            ...
                            <FormGroup row>
                                ...
                                >
                                    <FormLabel component='legend'>&nbsp;&nbsp;High *&nbsp;&nbsp;</FormLabel>
                                    <FormGroup row>
                                        <input
                                            type='color'
                                            onChange={handleOnChange}
                                ...
                                <FormControl
                                    ...
                                    error={Boolean(formErrors.moderateActivitySnapshotCategoryColor)}
                                >
                                    <FormLabel component='legend'>&nbsp;&nbsp;Moderate *&nbsp;&nbsp;</FormLabel>
                                    <FormGroup row>
                                        <input
                                            type='color'
                                            onChange={handleOnChange}
                                ...
                                >
                                    <FormLabel component='legend'>&nbsp;&nbsp;Low *&nbsp;&nbsp;</FormLabel>
                                    <FormGroup row>
                                        <input
                                            type='color'
                                            onChange={handleOnChange}
                            ...
                            Boolean(formErrors.moderateActivitySnapshotCategoryColor) ||
                            Boolean(formErrors.highActivitySnapshotCategoryColor) ? (
                                <FormLabel component='legend' className='category-colors-error-message'>
