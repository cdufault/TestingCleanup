...
export default function ConfigurationPage() {
	...
	const gateConfigured = useAppSelector(
		(state) => state.applicationSlice.gateConfigured,
	);
	const appLabel = useAppSelector(
		(state) => state.applicationSlice.applicationConfig?.appLabel ?? "",
	);
	...
	const [message, setMessage] = useState(
		`${joinLabel(
			...
			appLabel ? appLabel + " or " : ""
		}Esri Portal administrator.`,
	);
	...
	async function initialize() {
		const adminGroupMembers = await getPortalGroupMembers(
			portal.restUrl,
			userSession,
			appConfig.gateAdminGroupId,
		);
		const groupMembers = [
			...adminGroupMembers.users,
		...
		setMessage(
			isAdmin
				...
				: `The user [${
						user.username
				  }] does not have sufficient priviledges to configure${
						appLabel ? " " + appLabel : ""
				  }. Contact your ${
						appLabel ? appLabel + " or " : ""
				  }Esri Portal administrator.`,
	...
	async function handleOnClick() {
		...
		try {
			...
			if (shareResult.error) {
				Logger.log(
					"Error Sharing GATE admin Application.",
					"ERROR",
					shareResult,
				);
				navigate("/error", {
					state: "failed to share GATE Config Application.",
				});
			}
					...
					<Box className="ConfigurationPage-body">
						<Box className="gate-config-article">
							...
							<Typography variant="h6" align="center">
								...
								this idea was not a practical deterrent for reasons which at
								this moment must be all too obvious." - Dr. Strangelove
							</Typography>
						</Box>
					</Box>
				)}
