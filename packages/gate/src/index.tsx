...
import { DefaultPortal, getPortalItemDataById, getPortalUserSession } from '@stratcom/lib-functions';
    ...
    try {
        ...
        if (process.env.NODE_ENV === 'production') {
            ...
            const basenameIndex = pathParts.findIndex(
                (part) => part === process.env.REACT_APP_EXERCISE_BASENAME || part === process.env.REACT_APP_BASENAME
            );
            if (basenameIndex !== -1) {
        ...
        try {
            ...
            try {
                ...
                if (!portal) {
                    return new Error('Unable to get the Default Portal from portalUrl = ' + portalUrl);
                }
                const userSession = await getPortalUserSession(portalUrl, oauthAppId);
                if (!userSession) {
                    return new Error('Unable to get the userSession from Portal using portalUrl = ' + portalUrl);
                }
                ...
                if (gateWebApp) {
                    ...
                    const itemData = await getPortalItemDataById(gateWebApp.id, portalUrl, appConfig.oauthAppId);
                ...
                errorMessage = 'Error occurred initializing the app see object for more details.';
                throw e;
            }
        } catch (fetchError) {
